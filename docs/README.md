# Brotherizer Docs

This repo uses an in-repo wiki layout on purpose.

Why:

- Git history stays with the docs
- product changes and doc changes can be reviewed together
- the public story stays in the repo instead of drifting into a detached wiki

The `docs/wiki/` folder is also structured to be easy to mirror into a GitHub Wiki later if you want the public docs in both places.

## Start here

- [Start Here](wiki/START_HERE.md)
- [How It Works](wiki/HOW_IT_WORKS.md)
- [Positioning](wiki/POSITIONING.md)

## Product docs

- [API Reference](wiki/API_REFERENCE.md)
- [Modes and Surfaces](wiki/MODES_AND_SURFACES.md)
- [Model Routing and Providers](wiki/MODEL_ROUTING_AND_PROVIDERS.md)
- [Local Setup and Databases](wiki/LOCAL_SETUP_AND_DATABASES.md)
- [Security and Secrets](wiki/SECURITY_AND_SECRETS.md)
- [FAQ](wiki/FAQ.md)

## Research and corpus-building

- [`RESEARCH/README.md`](../RESEARCH/README.md)
- [`RESEARCH/BUILDING_DATABASES.md`](../RESEARCH/BUILDING_DATABASES.md)
- [`RESEARCH/DONOR_PACKS.md`](../RESEARCH/DONOR_PACKS.md)
- [`RESEARCH/STYLE_RADAR.md`](../RESEARCH/STYLE_RADAR.md)
- [`RESEARCH/CONTRIBUTING.md`](../RESEARCH/CONTRIBUTING.md)
- [`RESEARCH/LANGUAGE_COVERAGE.md`](../RESEARCH/LANGUAGE_COVERAGE.md)

## Suggested reading order

1. Start with [Start Here](wiki/START_HERE.md).
2. Read [How It Works](wiki/HOW_IT_WORKS.md).
3. If you are integrating the runtime, jump to [API Reference](wiki/API_REFERENCE.md).
4. If you are building packs or databases, jump to [`RESEARCH/README.md`](../RESEARCH/README.md).
